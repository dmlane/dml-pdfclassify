...
def get_devonthink_groups(force_refresh: bool = False) -> List[str]:
    ...
    try:
        ...
        groups = [g.strip() for g in result.stdout.split(",") if g.strip()]
...
class LabelBoostCLI:
    ...
    def _edit_label(self, label: str) -> None:
        ...
        refresh = questionary.confirm("Refresh DEVONthink group list?", style=CUSTOM_STYLE).ask()
        dt_groups = get_devonthink_groups(force_refresh=refresh)
        if dt_groups:

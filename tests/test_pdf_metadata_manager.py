...
def test_write_and_read_custom_metadata(sample_pdf_file: Path) -> None:
...
def test_write_metadata_without_overwrite(sample_pdf_file: Path) -> None:
...
def test_delete_custom_metadata(sample_pdf_file: Path) -> None:
...
def test_timestamps_are_preserved(sample_pdf_file: Path) -> None:
    """Ensure file modification times are preserved after update."""
    original_stat = os.stat(sample_pdf_file)
    ...
    new_stat = os.stat(sample_pdf_file)
    assert new_stat.st_mtime == original_mtime


def test_print_metadata_smoke(sample_pdf_file: Path, capsys: pytest.CaptureFixture) -> None:
